...
// update Custom_get_ped_last_weapon_impact_coord && Custom_get_model_dimensions address in new updates
...
void GtavNativeLoadHook(unsigned int r3, unsigned int r4) {//
	...
	// TU12
	//func(0x83D040E8, r4, r5);

	// TU13
	//func(0x83D384E8, r4, r5);

	// TU15
	//func(0x83D1BAE8, r4, r5);

	// TU17
	//func(0x83D2C188, r4, r5);

	//TU18
	//func(0x83D50B58, r4, r5);

	//TU23
	//func(0x83DCC0D8, r4, r5);

	//TU24
	//func(0x83DCC9D8, r4, r5);

	//TU25
	func(0x83DDC8F8, r4, r5); //FIRST OFFSET RIGHT
...
void create_ambient_pickup(int Hash, float* Pos, int r5, int r6, int r7, int r8, int r9)
{
    ...
    VMArgs[2][7] = r8;
    VMArgs[2][8] = r9;
    ((void(*)(void *))FindNative(0x17B99CE7))(VMArgs);
    ResetStack();
}
...
//Any CREATE_AMBIENT_PICKUP(Hash pickupHash, float posX, float posY,
  //float posZ, int p4, int value, Hash modelHash, BOOL p7,
  //BOOL p8)
...
// 5293.14f, -5162.17f, 85.92f, 1.0f, GetHash("v_ilev_cd_sprklr"), GetHash("v_ilev_cd_sprklr_on"), 1 
...
void ADD_OWNED_EXPLOSION(int ped, PFLOAT Origin, int ExplosionType, float radius, bool playsound, bool novisual, float camshake)
{
	...
	VMArgs[2][5] = *(DWORD *)&(radius);
	VMArgs[2][6] = playsound ? 1 : 0;
	VMArgs[2][7] = novisual ? 1 : 0;
	VMArgs[2][8] = *(DWORD *)&(camshake);
...
bool get_ped_last_weapon_impact_coord3(int ped, float *Outpos) {
	...
	Args[0] = ped;
	Args[1] = (DWORD)&Outpos[0];
	Args[2] = (DWORD)&Outpos[1];
	Args[3] = (DWORD)&Outpos[2];

	Pointer[0] = (DWORD)&Out;
	Pointer[1] = 4;
	Pointer[2] = (DWORD)Args;
	
	void (__cdecl *get_ped_last_weapon_impact_coord)(void *) = (void(__cdecl *)(void *))FindNative(0x9B266079);
	get_ped_last_weapon_impact_coord(Pointer);
	return Out != 0;
	/*SaveStack();
    VMArgs[2][0] = ped;
...
bool Custom_get_ped_last_weapon_impact_coord(int PedID, float* OutCoords) {
	...
	int prt = ((int(*)(int))customGetPedLastWeap->GetValue())(PedID);
	if(!prt || (prt = *(int *)(prt + 0xB8C)) == 0)
		return false;
	memcpy(OutCoords, (PVOID)(prt + 0xB0), 0xC);
	return true;
}

bool get_ped_last_weapon_impact_coord(int PedID, float* OutCoords)
{
    return Custom_get_ped_last_weapon_impact_coord(PedID, OutCoords);
}
...
bool get_ped_last_weapon_impact_coord2(int Ped, float *Co)
{
   SaveStack();
   VMArgs[2][0] = Ped;
   VMArgs[2][1] = (DWORD)&Co[0];
   VMArgs[2][2] = (DWORD)&Co[1];
   VMArgs[2][3] = (DWORD)&Co[2];
   ((void(*)(void*))FindNative(0x9B266079))(VMArgs);
   ResetStack();
   return VMArgs[0][0] != 0;
}
...
void get_model_dimensions(int model, float* Min, float* Max)
{
	...
	((void(*)(void *))FindNative(0x91ABB8E0))(VMArgs);
	ResetStack();*/
}
...
/*void get_blip_coords(int blip, float* OutPos)
{
    SaveStack();
    VMArgs[2][0] = blip;
    VMArgs[2][1] = (DWORD)OutPos;
    ((void(*)(void *))FindNative(0xEF6FF47B))(VMArgs);
    ResetStack();
	memcpy(OutPos, VMArgs[0], 0xC);
}

void get_blip_coords(int blip, float *x, float *y, float *z) {
	float OutPos[3];
	get_blip_coords(blip, OutPos);
	if(x)
		*x = *OutPos;
	if(y)
		*y = OutPos[1];
...
int START_ENTITY_FIRE(int entity){
	SaveStack();
	VMArgs[2][0] = entity;
    ((VOID(*)(PVOID))FindNative(0x8928428E))(VMArgs);
	ResetStack();
	return VMArgs[0][0];
}
...
void STOP_ENTITY_FIRE(int entity){
	SaveStack();
	VMArgs[2][0] = entity;
    ((VOID(*)(PVOID))FindNative(0xCE8C9066))(VMArgs);
	ResetStack();
}
...
int GET_ENTITY_ATTACHED_TO(int entity){
    SaveStack();
	VMArgs[2][0] = entity;
	((VOID(*)(PVOID))FindNative(0xFE1589F9))(VMArgs);
	ResetStack();	
	return VMArgs[0][0];
}
